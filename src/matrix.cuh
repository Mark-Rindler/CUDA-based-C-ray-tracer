...
public:
    ...
    __host__ __device__ mat3 inverse() const {
        ...
        if (fabsf(det) < 1e-6f) return mat3(); // Return identity if not invertible
...
__host__ __device__ inline mat3 scale(float s) {
    return scale(s, s, s);
}

// Rotation matrix from axis and angle
__host__ __device__ inline mat3 rotation_axis_angle(const vec3& axis, float angle) {
